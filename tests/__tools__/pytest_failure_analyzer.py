...
class FailureAnalyzer:
    ...
    def extract_failure_info(self, longrepr: str) -> Optional[Dict]:
        ...
        input_match = re.search(r"Input '([^']+)'", longrepr.replace("{input_text}", "").replace("'{'", "'").replace("'}'", "'"))
        if input_match and "{" in longrepr:
            # This is a formatted string pattern, extract the actual values
            expected_match = re.search(r"should (?:format to|be|protect|detect as|remain)[:\s]*'([^']+)'", longrepr.replace("{expected}", "").replace("'{'", "'").replace("'}'", "'"))
            actual_match = re.search(r"got '([^']+)'", longrepr.replace("{result}", "").replace("{actual}", "").replace("'{'", "'").replace("'}'", "'"))
        ...
        patterns = [
            ...
            (r"assert format_transcription\('([^']+)'\) == '([^']+)'",
             lambda m: {"input": m.group(1), "expected": m.group(2), "actual": self._extract_actual_from_assert(longrepr)}),
...
def pytest_addoption(parser):
    ...
    parser.addoption(
        "--summary-file",
        action="store",
        help="Output file for summary (default: test_summary.<format>)"
    )
